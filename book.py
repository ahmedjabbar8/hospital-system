...
def book():
    ...
    if request.method == 'POST':
        ...
        is_free     = 1 if visit_type in ['followup', 'lab_only', 'rad_only'] else 0
        ...
        try:
            ...
            # Lab/Rad go to their queue first (pending_lab_selection), others go to billing (scheduled)
            ...
            cursor.execute(
                ...
                (patient_id, f_doctor_id, f_dept_id, date, init_status, is_free)
            )
            ...
            if visit_type == 'lab_only':
                ...
                label = 'مراجعة مجانية' if visit_type == 'followup' else 'موعد جديد'
                flash(f"✅ تم حجز {label} بنجاح — المريض في قائمة انتظار المحاسبة.", "success")
                conn.close()
    ...
    # فحص الحجز المفتوح للمريض
    cursor.execute("""
        SELECT a.*, u.full_name_ar as doc_name, d.department_name_ar
        FROM appointments a
        LEFT JOIN users u ON a.doctor_id = u.user_id
        LEFT JOIN departments d ON a.department_id = d.department_id
        WHERE a.patient_id = %s AND a.status IN ('scheduled', 'in_progress')
    ...
    <style>
        ...
        [data-theme='dark'] .bk-clock-bar { background: rgba(30,15,40,0.65); border-color: rgba(255,255,255,0.07); }
        ...
        [data-theme='dark'] .bk-card { background: rgba(30,15,45,0.7); border-color: rgba(255,255,255,0.07); }
        ...
        [data-theme='dark'] .bk-section-title { color: #0a84ff; }
        ...
        [data-theme='dark'] .bk-field { background: rgba(255,255,255,0.05); border-color: rgba(255,255,255,0.1); color: #f0f0f0; }
        ...
        [data-theme='dark'] .vt-pill:checked + .vt-label { border-color: #0a84ff; background: rgba(10,132,255,0.15); color: #0a84ff; }
        ...
        [data-theme='dark'] .recent-appt-row { background: rgba(255,255,255,0.04); }
        .appt-status-badge {
    ...
    <div class="bk-wrap">
        ...
        <div class="row g-3">
            ...
            <div class="col-lg-7">
                    ...
                    <div class="d-flex align-items-start gap-3">
                        ...
                        <div class="flex-grow-1">
                            ...
                            <div style="font-size:0.8rem;opacity:0.7;margin-bottom:14px;">
                                لدى المريض موعد {{ 'قيد التنفيذ' if active_appt.status == 'in_progress' else 'مجدول' }} حالياً. يجب إلغاؤه أولاً قبل حجز موعد جديد.
                            </div>
                            <!-- Active appt details -->
                            <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:16px;">
                                ...
                                <div style="background:rgba(255,59,48,0.07);border-radius:10px;padding:8px 12px;border:1px solid rgba(255,59,48,0.15);">
                                    ...
                                    <div style="font-weight:800;font-size:0.82rem;" dir="ltr">
                                        {{ active_appt.appointment_date.strftime('%Y-%m-%d') if active_appt.appointment_date and active_appt.appointment_date.__class__.__name__ == 'datetime' else active_appt.appointment_date }}
                                    </div>
                                </div>
                            </div>
                            <div class="d-flex gap-2">
                                <button type="button" onclick="cancelActiveAppt({{ active_appt.appointment_id }})"
                                   style="display:inline-flex;align-items:center;gap:7px;padding:9px 18px;border-radius:10px;font-size:0.82rem;font-weight:800;background:#ff3b30;color:white;text-decoration:none;transition:all 0.2s;border:none;box-shadow: 0 4px 12px rgba(255,59,48,0.3);">
                                    <i class="fas fa-trash-alt"></i> إلغاء الحجز الحالي
                                </button>
                                <a href="/patients"
                                   style="display:inline-flex;align-items:center;gap:7px;padding:9px 18px;border-radius:10px;font-size:0.82rem;font-weight:700;border:1.5px solid rgba(255,59,48,0.3);color:#ff3b30;text-decoration:none;background:transparent;">
                ...
                <!-- Disabled Form -->
                <div style="opacity:0.35;pointer-events:none;user-select:none;" title="لا يمكن الحجز يوجد موعد مفتوح">
                {% else %}
                <div>
                {% endif %}

                <form method="POST" id="bookForm" {% if active_appt %}onsubmit="return false;"{% endif %}>
                    ...
                    <div class="bk-card">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-5">
                                ...
                                <input type="time" name="appt_time" class="bk-field" value="08:00" required>
                            </div>
                        </div>
                        <!-- Quick date pills -->
                    ...
                    <div class="bk-card">
                        ...
                        <div class="visit-types">
                            ...
                            <div>
                                <input type="radio" name="visit_type" value="lab_only" id="vt4" class="vt-pill" onclick="handleVisitTypeChange('lab_only')">
                            ...
                            <div>
                                <input type="radio" name="visit_type" value="rad_only" id="vt5" class="vt-pill" onclick="handleVisitTypeChange('rad_only')">
                        ...
                        <div class="recent-appt-row">
                            <div>
                                ...
                                <div style="opacity:0.45;font-size:0.68rem;margin-top:2px;">
                                    ...
                                    {{ a.appointment_date.strftime('%Y-%m-%d') if a.appointment_date and a.appointment_date.__class__.__name__ == 'datetime' else a.appointment_date }}
                                </div>
                            </div>

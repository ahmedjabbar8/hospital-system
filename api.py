...
def _fmt_time(dt):
    ...
    if isinstance(dt, str):
        try:
            ...
            s_dt = str(dt)
            return datetime.datetime.strptime(s_dt[0:19], '%Y-%m-%d %H:%M:%S').strftime('%I:%M %p') # type: ignore
        except Exception:
            return ''
    return ''
...
def api_get_signals():
    ...
    # Long-Polling Loop: Wait up to 5 seconds for a signal to arrive
    start_time = time.time()
    while time.time() - start_time < 5:
        # Create fresh cursor for each check
        ...
        if signals:
            # Delete and return immediately
            cursor.execute("DELETE FROM call_signaling WHERE receiver_id = %s", (my_id,))
            conn.commit()
            return jsonify(signals)
        
        conn.commit() # ensure we see updates
        time.sleep(0.4) # Wait short interval
        
    return jsonify([]) # Return empty if nothing arrived in 5s

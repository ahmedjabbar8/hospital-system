...
def add_patient():
    ...
    # --- SELF HEALING: Add columns if not exists ---
    try:
    ...
    if request.method == 'POST':
        ...
        if not error:
            sql = "INSERT INTO patients (file_number, full_name_ar, full_name_en, date_of_birth, gender, phone1, address, photo) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
            if conn.is_pg: sql += " RETURNING patient_id"

            try:
                cursor.execute(sql, (file_num, name, name_en, dob, gender, phone, address, photo_path))
    ...
    <style>
        ...
        [data-theme='dark'] .reg-photo-panel { background: rgba(30,15,40,0.5); }
        ...
        [data-theme='dark'] .reg-form-panel { background: rgba(30,15,40,0.5); }
            ...
            <div class="success-card shadow-lg text-center">
                ...
                <div class="patient-id-card-horiz">
                    ...
                    <div class="id-barcode-box-horiz">
                        ...
                        <div class="extra-small fw-bold opacity-50">{{ now.strftime('%Y-%m-%d') }}</div>
                    </div>
                </div>

...
from config import get_db, can_access, get_now, get_today_str # type: ignore
...
def consultation():
    ...
    if request.method == 'POST':
        if 'send_labs' in request.form:
            ...
            cursor.execute("UPDATE appointments SET status = 'completed', completed_at = %s WHERE appointment_id = %s", (get_now(), appt_id))
            conn.commit()
                                ...
                                <div class="hist-item mb-3 shadow-sm" style="border-right: 5px solid #667eea;">
                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                        ...
                                        <span class="text-muted small">
                                            {% if h.created_at and h.created_at.__class__.__name__ == 'datetime' %}{{ h.created_at.strftime('%Y-%m-%d | %I:%M %p') }}{% else %}{{ h.created_at }}{% endif %}
                                        </span>
                                    </div>
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr style="border-bottom: 1px solid #f1f5f9;" 
                                                        ...
                                                        <td class="py-3 text-muted small" style="font-size: 0.8rem;">
                                                            {{ lr.created_at.strftime('%Y-%m-%d') if lr.created_at and lr.created_at.__class__.__name__ == 'datetime' else lr.created_at }}
                                                        </td>
                                    ...
                                    {% for lr in profile_labs %}
                                        <div class="hp-card mb-3 p-4 border-0 shadow-sm" style="border-right: 8px solid #007aff; border-radius: 16px; background: #ffffff;">
                                            <div class="d-flex justify-content-between align-items-center mb-3">
                                                ...
                                                <span class="badge bg-light text-muted fw-normal" style="font-size: 0.75rem;"><i class="far fa-clock me-1"></i> {% if lr.created_at and lr.created_at.__class__.__name__ == 'datetime' %}{{ lr.created_at.strftime('%I:%M %p') }}{% endif %}</span>
                                            </div>
                        ...
                        <div class="col-12">
                            ...
                            <div id="rad-results-content" style="transition: all 0.4s ease-in-out;">
                                ...
                                {% for rr in rad_history %}
                                    {% set cur_date = rr.created_at.strftime('%Y-%m-%d') if rr.created_at and rr.created_at.__class__.__name__ == 'datetime' else rr.created_at %}
                                    {% if cur_date and 'CURRENT' not in cur_date|string %}
                                    ...
                                    <div class="hp-card mb-3 p-4 border-0 shadow-sm" style="border-right: 8px solid #ff2d55; border-radius: 16px; background: #ffffff;">
                                        <div class="d-flex justify-content-between align-items-center mb-3">
                                            ...
                                            <span class="badge bg-light text-muted fw-normal" style="font-size: 0.75rem;"><i class="far fa-clock me-1"></i> {% if rr.created_at and rr.created_at.__class__.__name__ == 'datetime' %}{{ rr.created_at.strftime('%I:%M %p') }}{% endif %}</span>
                                        </div>
    ...
    followup_date_val = (get_now() + datetime.timedelta(days=7)).strftime('%Y-%m-%d')

...
def login():
    ...
    <head>
        ...
        <style>
            :root {
                --primary: #d07afb;
                --bg: #000000;
                --card: rgba(26, 11, 29, 0.6);
                --input: rgba(0, 0, 0, 0.3);
                --text: #ffffff;
                --border: rgba(208, 122, 251, 0.15);
            ...
            * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Lexend', sans-serif; transition: background 0.4s, color 0.4s; }
            ...
            .theme-toggle { cursor: pointer; font-size: 1.2rem; color: var(--primary); transition: 0.3s; padding: 10px; border-radius: 50%; }
            .theme-toggle:hover { background: rgba(0,0,0,0.05); transform: rotate(20deg); }
            ...
            .login-container.ready { opacity: 1; filter: blur(0); transform: scale(1); }

            .brand { text-align: center; margin-bottom: 35px; }
            .brand i { font-size: 3.2rem; color: var(--primary); margin-bottom: 15px; filter: drop-shadow(0 0 15px var(--primary)); }
            ...
            .input-group-custom:focus-within { border-color: var(--primary); box-shadow: 0 0 15px rgba(var(--primary), 0.1); }
            ...
            .btn-auth:hover { transform: translateY(-2px); filter: brightness(1.1); box-shadow: 0 10px 20px rgba(var(--primary), 0.3); }
    ...
    <body dir="ltr">
        ...
        <div class="boot-screen" id="boot-overlay">
            <div class="hud-main-box">
                <svg class="hud-svg-core" viewBox="0 0 200 200">
                    <!-- Layer 1: Outer Dashboard Arcs -->
                    <circle class="l-outer-ring sys-color" cx="100" cy="100" r="95"></circle>
                    <circle class="l-outer-fine sys-color" cx="100" cy="100" r="95"></circle>
                    
                    <!-- Layer 2: Mid Indicator Track with cardinal nodes -->
                    <circle class="l-mid-track sys-color" cx="100" cy="100" r="82" stroke-dasharray="1 63.3"></circle>
                    <circle class="sys-color" cx="100" cy="100" r="82" opacity="0.1" stroke-width="0.5"></circle>
                    
                    <!-- Layer 3: Inner High-Density Dash Spinner -->
                    <circle class="l-inner-dash sys-color" cx="100" cy="100" r="66"></circle>
                    
                    <!-- Progress Arc tracking 0-100% -->
                    <circle id="p-arc" class="progress-glow-ring" cx="100" cy="100" r="90" style="transform: rotate(0deg);"></circle>
                </svg>
            ...
            <div class="boot-msg-bottom">SYSTEM INITIALIZING</div>
        </div>
        ...
        <script>
            ...
            function startBootProcess() {
                const valText = document.getElementById('p-val');
                const arcRing = document.getElementById('p-arc');
                const bootOverlay = document.getElementById('boot-overlay');
                const loginPanel = document.getElementById('login-panel');
                
                const circumference = 565; // PI * 2 * r(90)

                let countVal = 0;
                const procInterval = setInterval(() => {
                    countVal += Math.random() * 1.5;
                    if (countVal >= 100) {
                        countVal = 100;
                    ...
                    valText.textContent = Math.floor(countVal) + '%';
                    arcRing.style.strokeDashoffset = circumference - (circumference * countVal / 100);
                }, 40);
            }
            ...
            themeSwitcher.addEventListener('click', () => {
                ...
                themeSwitcher.innerHTML = nextT === 'dark' ? '<i class="fas fa-sun"></i>' : '<i class="fas fa-moon"></i>';
            });

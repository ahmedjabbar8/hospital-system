...
def patient_index():
    ...
    cursor.execute("SELECT COUNT(*) as p_count FROM patients WHERE DATE(created_at) >= %s", (start_date,))
    period_count = cursor.fetchone()['p_count']
    ...
    sql = """
        ...
        (SELECT MAX(appointment_date) FROM appointments WHERE patient_id = p.patient_id) as last_visit_date,
        (SELECT d.department_name_ar FROM appointments a JOIN departments d ON a.department_id = d.department_id WHERE a.patient_id = p.patient_id ORDER BY a.appointment_date DESC LIMIT 1) as last_dept
        FROM patients p 
    """
    ...
    if search:
        ...
        where_clauses.append("(p.full_name_ar LIKE %s OR p.full_name_en LIKE %s OR p.file_number LIKE %s OR p.phone1 LIKE %s OR p.national_id LIKE %s)")
        params.extend([search_term, search_term, search_term, search_term, search_term])
    if dept_id:
        where_clauses.append("p.patient_id IN (SELECT patient_id FROM appointments WHERE department_id = %s)")
        params.append(dept_id)
    if where_clauses:
        sql += " WHERE " + " AND ".join(where_clauses)
    sql += " ORDER BY last_visit_date DESC, p.created_at DESC LIMIT 100"
    ...
    <style>
        ...
        [data-theme='dark'] {
            --ui-canvas: #1a0b1d; 
            --ui-panel: rgba(255, 255, 255, 0.035);
            --ui-text: #ffffff;
            --ui-border: rgba(255,255,255,0.07);
            --ui-gray: rgba(255, 255, 255, 0.05);
            --ui-row-hover: rgba(255, 255, 255, 0.04);
            --ui-badge: rgba(216, 122, 251, 0.12);
        }
        ...
        [data-theme='dark'] .seg-btn.active { background: rgba(255,255,255,0.1); }
        ...
        [data-theme='dark'] .mock-table th { background: rgba(255,255,255,0.02); }

...
def patients():
    ...
    cursor.execute("""
        SELECT
            SUM(CASE WHEN DATE(appointment_date) = date('now')             THEN 1 ELSE 0 END) AS s_today,
            SUM(CASE WHEN DATE(appointment_date) = date('now','+1 day')    THEN 1 ELSE 0 END) AS s_tom,
            SUM(CASE WHEN strftime('%Y-%W', appointment_date) =
                          strftime('%Y-%W', date('now'))                   THEN 1 ELSE 0 END) AS s_week,
            SUM(CASE WHEN strftime('%Y-%m', appointment_date) =
                          strftime('%Y-%m', date('now'))                   THEN 1 ELSE 0 END) AS s_month
        FROM appointments
                        ...
                        <tbody>
                            {% for row in result %}
                                <tr>
                                    ...
                                    <td class="text-end pe-4"><i class="fas fa-chevron-left text-muted"></i></td>
                                </tr>
                            {% endfor %}
                        </tbody>
                    </table>

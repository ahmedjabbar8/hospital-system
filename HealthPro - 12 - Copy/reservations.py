...
def reservations():
    ...
    if filter_date == 'today':
        ...
        sql += " AND DATE(a.appointment_date) = date('now', '+1 day')"
    elif filter_date == 'week':
        sql += " AND strftime('%Y-%W', a.appointment_date) = strftime('%Y-%W', date('now'))"
    elif filter_date == 'month':
        sql += " AND strftime('%Y-%m', a.appointment_date) = strftime('%Y-%m', date('now'))"
    elif filter_date == 'upcoming':
        sql += " AND a.appointment_date >= date('now')"
    ...
    <div class="container-fluid py-4">
        ...
        <style>
            ...
            [data-theme='dark'] {
                ...
                --res-input: rgba(255,255,255,0.05);
            }
            ...
            [data-theme='dark'] .table thead th { background: rgba(255,255,255,0.03); }
        ...
        <div class="glass-card p-3 p-md-4">
            <form method="GET" class="row g-3" action="{{ url_for('reservations.reservations') }}">
                ...
                <!-- Filter Button -->
                <div class="col-md-2">
                    <button type="submit" class="btn btn-primary w-100 btn-pill shadow-sm fw-bold h-100 py-2">
                        ...
                        {% if result %}
                            {% for row in result %}
                                ...
                                <tr id="appt-{{ row.appointment_id }}">
                                    ...
                                    <td>
                                        <div class="fw-bold">
                                            {% if row.appointment_date and row.appointment_date.__class__.__name__ in ['datetime', 'date'] %}
                                                {{ row.appointment_date.strftime('%Y-%m-%d') }}
                                            {% elif row.appointment_date %}
                                                {{ row.appointment_date }}
                                            {% else %}
                                            {% endif %}
                                        </div>
                                        <div class="small text-muted">
                                            {% if row.created_at and row.created_at.__class__.__name__ == 'datetime' %}
...
def edit_reservation(id):
    ...
    <style>
        ...
        [data-theme='dark'] {
            ...
            --edit-card: #1a0b1d;
            --edit-text: #f0f0f0;
            --edit-border: rgba(255,255,255,0.1);
            --edit-input: rgba(255,255,255,0.05);
        }

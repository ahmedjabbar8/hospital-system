    ...
    if request.method == 'POST' and 'assign_tests' in request.form:
        ...
        if selected_tests and appt_id:
            for test_id in selected_tests:
                ...
                if t:
                    cursor.execute(
                        "INSERT INTO lab_requests (appointment_id, patient_id, doctor_id, test_type, price, status) VALUES (%s, %s, %s, %s, %s, 'pending_payment')",
                        (appt_id, patient_id, 1, t['test_name'], t['price'])
                    )

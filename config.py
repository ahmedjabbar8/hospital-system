...
# ── Dynamic Hospital Timezone (UTC + 3 for Iraq/Middle East) ───────────────
HOSPITAL_TZ_OFFSET = 3 

def get_now():
    """Returns the current localized hospital time."""
    return datetime.now(timezone(timedelta(hours=HOSPITAL_TZ_OFFSET)))
...
def update_last_activity(user_id):
    ...
    try:
        ...
        cur.execute(
            ...
            (get_now().strftime('%Y-%m-%d %H:%M:%S'), user_id)
        )
        db.commit()
        cur.close()
    except Exception:
